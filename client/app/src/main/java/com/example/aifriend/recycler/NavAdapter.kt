...
class NavAdapter(val context: Context, val itemList: List<UserData>): RecyclerView.Adapter<NavAdapter.MyViewHolder>() {
    ...
    override fun onBindViewHolder(holder: NavAdapter.MyViewHolder, position: Int) {
        ...
        holder.binding.textBtn.setOnClickListener{
            itemList[position].uid?.let { it1 -> itemList[position].name?.let { it2 ->
                createChat(it1,
                    it2
                )
            } }
...
//        }
    ...
    private fun createChat(user_uid: String, user_name: String) {
        /**
            ...
            fireStore?.collection("ChatRoomList")
                ...
                ?.addSnapshotListener { querySnapshot, firebaseFirestoreException ->
                    ...
                    for (snapshot in querySnapshot!!.documents) {
                        ...
                        if (item?.uid?.get(0).equals(user_uid)) {
                            chatList.add(item!!)
                            break
                        } else if (item?.uid?.get(1).equals(user_uid)) {
                            chatList.add(item!!)
                            break
                        } else {
                            continue
